    ...
    try {
        ...
        switch (command.on) {
            case "all":
                ...
                break;
            case "text":
                if (message.text) command.function(message, message.text, client);
                break;
            case "sticker":
                if (message.type === "stickerMessage") command.function(message, message.text, client);
                break;
            case "image":
                if (message.type === "imageMessage") command.function(message, message.text, client);
                break;
            case "video":
                if (message.type === "videoMessage") command.function(message, message.text, client);
                break;
            case "audio":
                if (message.type === "audioMessage") command.function(message, message.text, client);
                break;
            case "delete":
                if (message.type === "protocolMessage") {
                    message.messageId = message.message?.protocolMessage?.key?.id;
                    command.function(message, message.text, client);
                }
                break;
        }
