...
async function linkPreview(options = {}) {
    if (!config.LINK_PREVIEW || String(config.LINK_PREVIEW).toLowerCase() === 'false' || String(config.LINK_PREVIEW).toLowerCase() === 'null') {
        return undefined;
    }
    
    const previewParts = config.LINK_PREVIEW.split(/[;,|]/);

    const opt = {
        showAdAttribution: true,
        title: options.title || previewParts[0] || '𝗥𝗨𝗗𝗛𝗥𝗔 𝗕𝗢𝗧',
        body: options.body || previewParts[1] || '',
        mediaType: 1,
        thumbnailUrl: options.url || previewParts[2] || 'https://raw.githubusercontent.com/rudhraan/media/main/image/rudhra1.jpg',
        sourceUrl: previewParts[3] || 'https://whatsapp.com/channel/0029VasSpvKAe5Vp741msS3E'
    };
